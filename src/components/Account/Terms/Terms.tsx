...
export default function Terms() {
  ...
  return (
    <div className="mt-8 flex flex-col mb-12">
      {/* 전체 동의 버튼 */}
      <button
        type="button"
      ...
      >
        <div
          className={`flex h-5 w-5 items-center justify-center rounded-full border transition-colors 
            ${isAllChecked ? "border-black bg-black" : "border-[#E5E7EB] bg-white"}`}
        >
          {isAllChecked && (
            <Image src="/icon/check-icon.svg" alt="check-icon" width={12} height={12} />
          )}
        </div>
