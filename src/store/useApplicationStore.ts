...
export const useApplicationSaveAsDefault = () => {
  ...
  const setSelectedSaveDefault = useApplicationStore(
    (state) => state.setSelectedSaveDefault,
  );
  return {
    saveAsDefault,
...
export const useApplicationReset = () => {
  const setReset = useApplicationStore((state) => state.setReset);
  return { setReset };
};
